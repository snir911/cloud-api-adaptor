# peer-pod-config-controller
peer-pod-config-controller is a kubernetes controller which is responsible for
...
Running the peer-pod-config-controller as another controller embedded into an operator can be easily
...
```go
    import (
        ...
        peerpodcontrollers "github.com/confidential-containers/cloud-api-adaptor/peer-pod-config-controller/controllers"
        peerpodconfig "github.com/confidential-containers/cloud-api-adaptor/peer-pod-config-controller/api/v1alpha1"
        ...
    )
...
```sh
make docker-build docker-push IMG=<some-registry>/peer-pod-config-controller:tag
```
	
3. Deploy the controller to the cluster with the image specified by `IMG`:

```sh
make deploy IMG=<some-registry>/peer-pod-config-controller:tag
```
