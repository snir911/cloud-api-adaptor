...
# Deploy cloud-api-adaptor using the operator, pull image from $IMAGE_TAG if set
deploy:
	kubectl apply -f install/yamls/deploy.yaml
	kubectl apply -f install/examples/$(CLOUD_PROVIDER)-config-example.yaml
	cat install/yamls/ccruntime-peer-pods-in-pod.yaml | sed "s#quay.io/confidential-containers/cloud-api-adaptor:latest#${IMAGE_TAG}#g" | kubectl apply -f -

delete:
	kubectl delete -f install/yamls/ccruntime-peer-pods-in-pod.yaml
	kubectl delete -f install/examples/$(CLOUD_PROVIDER)-config-example.yaml
