...
func (r *PeerPodReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// cloud provider was not set, try to fetch cloud provider and its configs dyanmically from configmap or secret
	//(+)kubebuilder:rbac:groups="",namespace=xxxxxx,resources=configmaps;secrets,verbs=get
	if r.Provider == nil {
		if err := r.runtimeCloudConfig(); err != nil {
...
func (r *PeerPodReconciler) runtimeCloudConfig() error {
	...
	if err := r.Client.Get(context.TODO(), types.NamespacedName{Name: ppConfigMap, Namespace: ns}, &peerpodscm); err == nil {
		for k, v := range peerpodscm.Data {
			os.Setenv(k, v)
		}
	}

	if err := r.Client.Get(context.TODO(), types.NamespacedName{Name: ppSecret, Namespace: ns}, &peerpodssecret); err == nil {
	...
	if peerpodscm.Data == nil && peerpodssecret.Data == nil {
		return fmt.Errorf("could not fetch cloud configs")
	}
