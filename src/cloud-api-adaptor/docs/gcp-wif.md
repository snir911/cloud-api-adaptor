...
Deploy (assuming you have a separate values file for standard GCP configuration):

```bash
# Deploy with both your GCP config and WIF config
...
helm install peerpods ./install/charts/peerpods \
  ...
  --set gcp.workloadIdentityFederation.workloadPool=${PROJECT_ID}.svc.id.goog \
