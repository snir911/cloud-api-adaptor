...
Create a `gcp-wif-values.yaml` file using the environment variables from previous steps:

```bash
cat > gcp-wif-values.yaml <<EOF
provider: gcp

# WIF-specific configuration - only needed when using Workload Identity Federation
gcp:
...
# Standard GCP provider configuration (required regardless of auth method)
providerConfigs:
  gcp:
    GCP_PROJECT_ID: "${PROJECT_ID}"
    GCP_ZONE: "${ZONE}"
    PODVM_IMAGE_NAME: "projects/${PROJECT_ID}/global/images/podvm-image"
    # Optional configurations
    # PODVM_INSTANCE_TYPE: "n2d-standard-2"
    # DISABLECVM: "false"

# When using WIF: do NOT set GCP_CREDENTIALS in providerSecrets
# The WIF credentials (GOOGLE_APPLICATION_CREDENTIALS_JSON) are auto-generated by the chart
secrets:
  mode: "create"
EOF
```
