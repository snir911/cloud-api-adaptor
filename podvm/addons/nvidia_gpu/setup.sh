...
rhel_kernel_version_matching() {
    ...
    local latest_nv_version=$(repoquery --latest-limit 1 nvidia-driver  --queryformat "%{version}" -q) # get latest nvidia driver available version
    local latest_kmod_pkg_name=$(repoquery --latest-limit 1 kmod-nvidia-${latest_nv_version}-* --queryformat "%{name}" -q | sort -V | tail -n 1) # get latest version of kmod-nvidia pkg name
    local matching_kernel_version=$(echo ${latest_kmod_pkg_name} | sed "s/kmod-nvidia-${latest_nv_version}-//") # extract matching kernel version from kmod-nvidia name
    ...
    dnf -y install kernel-${matching_kernel_version}*  kernel-core-${matching_kernel_version}*  kernel-modules-core-${matching_kernel_version}*  kernel-modules-${matching_kernel_version}*
    if [ $? -eq 0 ]; then
        return 0
    else
        dnf -y update kernel kernel-core kernel-modules-core kernel-modules
    fi
}
